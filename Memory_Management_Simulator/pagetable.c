...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		if ((victim->frame & PG_ONSWAP) != 0 && (victim->frame & PG_DIRTY) == 0) {
			...
			success = swap_pageout(frame, victim->swap_off);
			evict_dirty_count++;

			if (success == INVALID_SWAP) {
				perror("Invalid swap.\n");
				exit(1);
			} else {
				victim->swap_off = success; /*Assign offset in swap file to the victim.*/
			}
		}
		victim->frame = (victim->frame & ~PG_VALID); /*The victim is not valid anymore.*/
	}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	if ((pgdir_entry & PG_VALID) == 0) {
		//Invalid pointer
		pgdir[idx] = init_second_level();
		pgdir_entry = pgdir[idx].pde;
	}

	// Use vaddr to get index into 2nd-level page table and initialize 'p'
	unsigned entry = PGTBL_INDEX(vaddr);
	p = (pgtbl_entry_t *)(pgdir_entry & PAGE_MASK) + entry; //Move the pointer p to the correct index in 2nd-level page table
	...
	if ((p->frame & PG_VALID) == 0 && (p->frame & PG_ONSWAP) == 0) {
		...
		p->frame = p->frame | PG_DIRTY; //the page entry is dirty when it is not on swap
		miss_count++;
		ref_count++;
	} else if ((p->frame & PG_VALID) == 0 && (p->frame & PG_ONSWAP) != 0) {
		//Invalid and on swap
		int frame_number = allocate_frame(p);
		int success = swap_pagein(p->frame >> PAGE_SHIFT, p->swap_off);
		if (success == -1) {
			perror("swap page in\n");
			exit(1);
		} else {
		...
		p->frame = frame_number << PAGE_SHIFT;
		p->frame = p->frame | PG_ONSWAP; //restore original onswap status
		miss_count++;
		ref_count++;
	} else {
		//Valid
